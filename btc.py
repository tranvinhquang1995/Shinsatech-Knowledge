...
def fetch_btc_data(sheet_url: str) -> str:
    ...
    QC Note: Sử dụng Regex để auto-convert định dạng URL, ép Google Sheet trả file CSV.
    Save effort cho team không cần thao tác tạo link thủ công.
    """
    try:
...
def get_btc_chat_session(api_key: str, context: str):
    """
    Khởi tạo phiên Chat (Multi-turn) độc lập cho team BTC.
    """
    ...
    3. Nếu câu hỏi của User chứa NHIỀU vấn đề khác nhau, hãy bóc tách và trả lời từng vấn đề một cách tuần tự dựa trên các hàng tương ứng trong [NGUỒN DỮ LIỆU BTC]. Sử dụng gạch đầu dòng rõ ràng cho từng vấn đề được giải quyết.
